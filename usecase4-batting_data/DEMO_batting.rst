...
- 다운받은 zip 을 풀어서 미국 야구의 타격 데이터인 Batting.csv 만 IRIS 에 업로드하여 타격 지표와 챠트를 만들어 봅니다.
...
- 로컬 PC 에 다운로드된  Batting.csv 파일을 IRIS 의 **HDFS브라우저** 를 이용하여 minio 에 업로드합니다.
    - minio 홈페이지 `MinIO <https://min.io/>`__
...
- 데이터모델 DEMO_BATTING_NEW 를 생성합니다.

.. image:: ../images/demo/demo_batting_07.png
    :alt: 데이터 - 07


- 검색 메뉴에서 데이터 모델 DEMO_BATTING_NEW 를 조화해 봅니다.
...
- 연도(yearID)별 데이터 
    - 콤보박스에서 선택된 연도의 데이터가 5개의 챠트에 그려집니다.
...
- 조회할 연도의 Batting 데이터를 **챠트** 의 테이블로 출력하는 과정입니다.(2)
    ...
    - 검색어 구문 해석  : `검색어에 대한 설명 <http://docs.iris.tools/manual/IRIS-Manual/IRIS-Discovery-Middleware/command/index.html>`__
    - 각 오브젝트에서 변수명 확인하는 방법 : `전체 변수명 보기`_
...
- 3개의 챠트는 모두 같은 방식으로 생성되므로 경기당 홈런수/안타수 챠트를 예로 들어 보이겠습니다.
